...
Cube.js provides security context functionality, which can be used in the
...
By default, SQL API uses current user's security context, but there is way how to change it.

First, you need to define what user is allowed to change security context:

```
CUBEJS_SQL_SUPERUSER=admin
```
