...
All Cube database drivers come with presets for concurreny and pooling that work out-of-the-box.
The following information is included as a reference.

</InfoBox>

For increased performance, Cube uses multiple concurrent connections to configured
data sources.

Internally, Cube uses two queues that manage database requests; one
for API queries and one for pre-aggregations. The number of concurrent database
connections for each queue can be set by the `CUBEJS_CONCURRENCY` environment variable.
To limit the maximum number of concurrent connections to the database, the `CUBEJS_DB_MAX_POOL`
