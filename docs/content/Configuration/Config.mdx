...
interface CubejsConfiguration {
  ...
  canSwitchSqlUser: (current: string, user: string) => bool;
...
Since v0.30.30 `dbType` usage is discouraged and substituted with a unified [driverFactory][self-driver-factory] implementation that returns `DriverConfig` object.
Please consider defining [driverFactory][self-driver-factory] that returns `DriverConfig` object instead.
...
Called once per [`dataSource`][ref-schema-ref-datasource].
Can return a `Promise` which resolves to a `DriverConfig`.
`DriverConfig` consists of a `type` field corresponding to database type and options passed to a driver constructor.
...
Used in [SQL API][ref-sql-api], and can be an `async` function. Default
implementation depends on `CUBEJS_SQL_SUPERUSER` and return `true` when it's equal to session's user.
...
```javascript
module.exports = {
  canSwitchSqlUser: (current, username) => {
    if (current === 'admin') {
        return true;
    }
